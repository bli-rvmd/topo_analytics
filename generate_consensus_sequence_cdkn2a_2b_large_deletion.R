...
## FIXME! ToDo - implement loading fastq file (which can be generated by BioPython directly from *.ab1 files) and trim each sequence in it instead of loading *.seq files
...
## data folder 
# data_folder <- "/Volumes/revmed/Shared/R&D/Biology/Translational Research/Model Engineering Core/Lab/Data/Sequencing/Azenta/Revolution Medicines_Zhang_30-1040385835_ICE Results/TA Cloning Data/Final Data/Trp53/"
data_folder <- "./data/KPTC_ElimBio_Order_865179_20241029_Cdkn2a_Cic_2D/"
sub_data_folder <- "pcr_60_cic"
delim_clone <- c("_2-", "_M13-") ## delimiters identifying clone number

## PCR primers cdkn2a primers 17 and 18
# fwd_primer <- toupper("cacacaatcccagttcggct")
# rev_primer <- toupper("gcagaactgaatgaatgttagacct")

## PCR primers cic primers 1 and 2
# fwd_primer <- toupper("GTGTTAACTGCAGCGGGAGA")
# rev_primer <- toupper("TCGGTCCTTGTTGCACCATT")

## PCR primers cic primers 3 and 4
fwd_primer <- toupper("TCTCCACAGCTTCCTCTCCA")
rev_primer <- toupper("GTGGCTATGGTGCCATGACT")

## PCR primers cdkn2a - cdkn2b large deletion double sgRNA pcr_19
# fwd_primer <- toupper("TCAGGCTCATTTGGGTTGCT")
# rev_primer <- toupper("AAGGGAACCGATCCAATCCG")

## PCR primers smad4 - primers 15 and 16
...
for (c_id in clone_ids) {
  ...
  for (seq_file in c_seq_files) {
    ...
    s <- toupper(paste(seqinr::read.fasta(file = seq_file)[[1]], collapse = ""))
...
########### TEMP TEST #############
# 
# create_fastq_file_from_seqs(list_clone_seqs$forward_in_rc_reverse_in, output_file_name = paste0(data_folder, sub_data_folder, "/", sub_data_folder, "_r1_seqs_both_primers_in.fastq"))
# create_fastq_file_from_seqs(list_clone_seqs$reverse_in_rc_forward_in, output_file_name = paste0(data_folder, sub_data_folder, "/", sub_data_folder, "_r2_seqs_both_primers_in.fastq"))
